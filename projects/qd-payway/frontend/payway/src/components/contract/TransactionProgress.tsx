...
export function TransactionProgress({ 
  ...
  if (step === TransactionStep.COMPLETED) {
    return (
      <Alert className="border-emerald-200 bg-emerald-50">
        ...
        <AlertTitle className="text-emerald-900">创建成功！🎉</AlertTitle>
        <AlertDescription className="text-emerald-800">
          <p className="mb-2">
            托管订单已成功创建，资金已安全锁定在智能合约中。
          </p>
          {transactionHash && (
            <Button
              variant="outline"
  ...
  return (
    <Alert className="border-teal-200 bg-teal-50">
      ...
      <AlertTitle className="text-teal-900">交易进行中</AlertTitle>
          ...
          {transactionHash && (
            <div className="mt-2 pt-2 border-t border-teal-200">
              <a
                ...
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-1 text-sm text-teal-700 hover:text-teal-900 hover:underline"
              >
                查看交易详情
                <ExternalLink className="h-3 w-3" />
              </a>
