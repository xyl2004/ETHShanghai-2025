...
import { IDiamond, Diamond, RouterManagementFacet } from "@/types/index";

describe("RouterManagementFacet.spec", async () => {
  let deployer: HardhatEthersSigner;
  let signer: HardhatEthersSigner;

  let diamond: Diamond;
  let facet: RouterManagementFacet;
  ...
  beforeEach(async () => {
    ...
    for (const name of ["DiamondCutFacet", "DiamondLoupeFacet", "OwnershipFacet", "RouterManagementFacet"]) {
    ...
    facet = await ethers.getContractAt("RouterManagementFacet", await diamond.getAddress(), deployer);
  });
  ...
  context("updateWhitelist", async () => {
    it("should revert when caller is not owner", async () => {
      await expect(facet.connect(signer).updateWhitelist(ZeroAddress, true))
        .to.revertedWithCustomError(facet, "NotContractOwner")
        .withArgs(signer.address, deployer.address);
    });

    it("should succeed", async () => {
      expect(await facet.getWhitelisted()).to.deep.eq([]);
      await facet.updateWhitelist(deployer.address, true);
      expect(await facet.getWhitelisted()).to.deep.eq([deployer.address]);
    });
  });
