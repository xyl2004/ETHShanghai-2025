...
describe("PositionOperateFlashLoanFacetV2.spec", async () => {
  ...
  beforeEach(async () => {
    ...
    // deploy Facets and Diamond
    {
      ...
      const PositionOperateFlashLoanFacetV2 = await ethers.getContractFactory(
      ...
      const flashSwapV2 = await PositionOperateFlashLoanFacetV2.deploy(
        "0xbbbbbbbbbb9cc5e90e3b3af64bdaf62c37eeffcb",
        poolManager.getAddress()
      );
      ...
      const diamondCuts = [
        ...
        },
        {
          ...
          action: 0,
          functionSelectors: getAllSignatures(flashSwap.interface),
        },
        {
          facetAddress: await migrate.getAddress(),
          action: 0,
          functionSelectors: getAllSignatures(migrate.interface),
        },
        {
        ...
        {
          facetAddress: await flashSwapV2.getAddress(),
          action: 0,
          functionSelectors: getAllSignatures(flashSwapV2.interface),
        },
      ];
  ...
  const openOrAdd = async (
    ...
    const facet = await ethers.getContractAt("PositionOperateFlashLoanFacetV2", await router.getAddress(), deployer);
    ...
    await facet.connect(holder).openOrAddPositionFlashLoanV2(
      {
  ...
  const closeOrRemove = async (
    ...
    await pool.connect(holder).approve(facet.getAddress(), positionId);
    const tx = await facet.connect(holder).closeOrRemovePositionFlashLoanV2(
      {
