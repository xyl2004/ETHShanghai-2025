...
contract IdentityRegistry is ERC721URIStorage, Ownable {
    ...
    function register() internal returns (uint256 agentId) {
        agentId = _lastId++;
        _safeMint(msg.sender, agentId);
        emit Registered(agentId, "", msg.sender);
    }

    function register(string memory tokenUri) internal returns (uint256 agentId) {
        agentId = _lastId++;
        _safeMint(msg.sender, agentId);
    ...
    function register(string memory tokenUri, MetadataEntry[] memory metadata) internal returns (uint256 agentId) {
        agentId = _lastId++;
        _safeMint(msg.sender, agentId);
        _setTokenURI(agentId, tokenUri);
        emit Registered(agentId, tokenUri, msg.sender);
