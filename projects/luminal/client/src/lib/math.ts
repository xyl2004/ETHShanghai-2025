import type { PoolState, SwapComputation } from "./types";
...
export const formatBigint = (value: bigint, decimals: number, precision = 4) => {
...
export const computeSwapState = (state: PoolState, amountIn: bigint) => {
  const amountInAfterFee =
    (amountIn * (BPS_DIVISOR - state.feeBps)) / BPS_DIVISOR;
...
export const calculateSwap = async (
  state: PoolState,
  amountIn: bigint
): Promise<SwapComputation> => {
  const { amountInAfterFee, amountOut, newState } = computeSwapState(
    state,
    amountIn
  );
...
export const previewSwap = (state: PoolState, amountIn: bigint) => {
  return computeSwapState(state, amountIn);
};

export const calculatePriceImpactBps = (
  state: PoolState,
  newState: PoolState
) => {
  ...
  const impact = ((spotBefore - spotAfter) / spotBefore) * 10_000;
