...
export const generateSwapProof = async (input) => {
    const { proof, publicSignals } = await groth16.fullProve(input, CIRCUIT_WASM_PATH, CIRCUIT_ZKEY_PATH);
    return {
        proof,
        publicSignals: publicSignals
    };
};
export const packProofForContract = async (proof) => {
