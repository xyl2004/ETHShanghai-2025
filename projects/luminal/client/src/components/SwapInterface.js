...
export function SwapInterface() {
    ...
    const { poolState, commitment, isLoadingState, error } = usePoolState();
    ...
    const statusMessage = useMemo(() => {
        if (!appConfig.ammAddress) {
            return "Set VITE_AMM_CONTRACT_ADDRESS (and optionally VITE_VAULT_CONTRACT_ADDRESS) in the client .env to load pool state.";
        }
        if (swap.error)
            return swap.error;
        if (error)
            return error.message;
        if (!poolState && !isLoadingState)
            return "Unable to decrypt pool state. Check viewing key or state service.";
        return null;
    }, [swap.error, error, poolState, isLoadingState]);
