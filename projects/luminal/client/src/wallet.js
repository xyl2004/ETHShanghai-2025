...
const disableWalletConnect = import.meta.env.VITE_DISABLE_WALLETCONNECT?.trim().toLowerCase() === "true";
...
const resolvedChains = (useAnvil ? [anvil] : [sepolia]);
const transports = resolvedChains.reduce((map, chain) => {
...
if (!shouldUseWalletConnect && !projectId) {
    ...
    console.info("WalletConnect project id not provided. Falling back to injected wallets only.");
}
