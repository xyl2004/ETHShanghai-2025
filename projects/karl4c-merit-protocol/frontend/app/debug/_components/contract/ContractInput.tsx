    ...
    switch (paramType.type) {
      ...
      default:
        ...
        if (paramType.type.includes("int") && !paramType.type.includes("[")) {
          // Check if this is a timestamp field (lastUpdated, timestamp, etc.)
          const isTimestampField = paramType.name?.toLowerCase().includes("timestamp") || 
                                   paramType.name?.toLowerCase().includes("lastupdated") ||
                                   paramType.name?.toLowerCase().includes("updatedat");
          
          if (isTimestampField) {
            return <TimestampInput {...inputProps} variant={paramType.type as IntegerVariant} />;
          }
