...
export default function AssetCards({ assets, onSwap, onBuild }) {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
      {assets.map((a) => (
        <div key={a.assetId || a.id} className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm hover:shadow-md transition-shadow">
          <div className="flex items-start justify-between gap-3">
            <div className="min-w-0 flex-1">
              ...
              <div className="text-xs text-slate-500 mt-1 truncate whitespace-nowrap" title={`Issuer: ${a.issuer} · Maturity: ${formatDateToYMD(a.maturity)}`}>
                Issuer: {a.issuer} · Maturity: {formatDateToYMD(a.maturity)}
              </div>
            ...
            <div className="text-right flex-shrink-0">
              ...
              <div className="text-lg font-semibold whitespace-nowrap">${a.tvl.toFixed(1)}</div>
              <div className="text-xs text-slate-500 whitespace-nowrap">24h ${a.vol24h.toFixed(1)}</div>
            </div>
          </div>
          ...
          <div className="mt-3 grid grid-cols-3 gap-2">
            <PCSPanel
              label="P"
              value={`${a.pApr.toFixed(1)}%`}
              sub="Maturity APR"
              tone="info"
              onClick={() => onSwap({ assetId: a.assetId, from: "USDC", to: `${a.assetId}:P`, action: "buy" })}
            />
            <PCSPanel
              label="C"
              ...
              sub="Coupon APR"
              tone="success"
              onClick={() => onSwap({ assetId: a.assetId, from: "USDC", to: `${a.assetId}:C`, action: "buy" })}
            />
            <PCSPanel
              label="S"
              value={`${avg(a.sAprRange).toFixed(1)}%`}
              sub={`${a.sAprRange[0]}%~${a.sAprRange[1]}%`}
              tone="warn"
              onClick={() => onSwap({ assetId: a.assetId, from: "USDC", to: `${a.assetId}:S`, action: "buy" })}
            />
          </div>

          {/* <div className="mt-3 flex items-center justify-center">
            <button
              onClick={() => setOpenSimId(openSimId === (a.assetId || a.id) ? null : (a.assetId || a.id))}
              className="text-sm text-slate-600 hover:text-slate-800"
            >
              {openSimId === (a.assetId || a.id) ? "Hide Portfolio Purchase" : "Portfolio Purchase (Slider)"}
            </button>
          </div> */}
          ...
          {openSimId === (a.assetId) && (
            <MiniSimulator
              asset={a}
              onQuickSwap={(leg) => onSwap({ assetId: a.assetId, from: "USDC", to: `${a.assetId}:${leg}`, action: "buy" })}
            />
          )}
